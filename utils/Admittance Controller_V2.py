...
def admittance(target_force, wrench, des_vel, rot_align, Xc, current_vel):
    ...
    #Desired frame calculation (V1: just same z, small step in x,y direction)
    Xd = np.copy(Xc) + np.array([0.01, 0.01, 0])

    pos_error = Xc - Xd #desired frame initializes as current frame, so first time pos_error = 0
    vel_error = current_vel - des_vel #desired velocity initializes as initial velocity, so first time vel_error = 0
    
    # Calculate acceleration
    acc = np.linalg.inv(M) * (wrench + target_force - D @ vel_error - K @ pos_error) 
